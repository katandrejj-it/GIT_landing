// ==UserScript==
// @name         Visual Designer Tool v8.2
// @namespace    http://tampermonkey.net/
// @version      8.2
// @description  Professional visual designer: gradient-aware editing, CSS var detect + unlink, correct color reading, import/export
// @author       Andrei K. / Claude
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  function scanAllCSSVars() {
    ...
    for (const sheet of document.styleSheets) {
      ...
      for (const rule of rules) {
        ...
        const isDark = sel.includes('.dark') || sel.includes('[data-theme="dark"]') || sel.includes(':root.dark') || sel.includes('html.dark');
        ...
        if (isRoot || isDark) {
          for (const prop of rule.style) {
            ...
            if (!allFound.has(prop)) allFound.set(prop, { light: null, dark: null });
  ...
  function getElementColors(el) {
    ...
    const bgVar     = detectCSSVar(el, 'background-color') || detectCSSVar(el, 'background');

    // Transparency: computed style collapses rgba alpha too
    const bgTransparent = !bgColor || bgColor === 'transparent' || bgColor === 'rgba(0, 0, 0, 0)';
    ...
    return {
      bgIsGradient:   bgImage && bgImage !== 'none' && bgImage.includes('gradient'),
      ...
      bgHasAlpha,
      color,
      textIsGradient: webkitFill === 'transparent' || webkitFill === 'rgba(0, 0, 0, 0)',
      colorVar,
      bgVar,
      boxShadow
    };
  ...
  function removeOverride(sel, prop) {
    changes.overrides = changes.overrides.filter(o => !(o.selector === sel && o.prop === prop));
    rebuildInjectCSS();
  }
  ...
  overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;z-index:2147483646;display:none;cursor:crosshair;';
  ...
  overlay.addEventListener('click', e => {
    ...
    overlay.style.pointerEvents = '';
    if (!el || el.closest('#' + PANEL_ID)) return;
    if (lastHovered) { lastHovered.style.removeProperty('outline'); lastHovered = null; }
    stopModes();
  ...
  /* ═══════════════════════════════════════════════════════════
     TABS
  ═══════════════════════════════════════════════════════════ */
  const panes = { globals: 'pane-globals', manual: 'pane-manual', colors: 'pane-colors' };
  ...
  shadow.querySelectorAll('.tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      Object.values(panes).forEach(id => { shadow.getElementById(id).style.display = 'none'; });
  ...
  function renderVarList(container, allVars, isDark) {
    ...
    for (const [name, entry] of allVars) {
      const rawVal = entry[isDark ? 'dark' : 'light'] ||
      ...
      if (isGradVar) {
        ...
        if (parsed) {
          ...
          parsed.stops.forEach((stop, i) => {
            const hex = stop.color.startsWith('#') ? stop.color : rgbToHex(stop.color);
            ...
            const update = nv => {
              ...
              cp.value = nv; tx.value = nv.toUpperCase();
  ...
  function buildEditor(el) {
    ...
    if (colors.bgIsGradient && colors.bgGradient) {
      ...
      if (parsed) {
        ...
        // Stop rows
        parsed.stops.forEach((stop, i) => {
          const hexVal = stop.color.startsWith('#') ? stop.color : rgbToHex(stop.color);
          const sr = document.createElement('div');
          sr.className = 'stop-row';
          sr.innerHTML = `
            <span class="stop-idx">#${i + 1}</span>
      ...
      if (colors.bgHasAlpha) {
        ...
        let currentBgHex = colors.bgColor ? rgbToHex(colors.bgColor) : '#000000';
        ...
        colorRow.innerHTML =
          ...
          '<div class="color-group">' +
            '<input type="text" class="hex" id="bg-hex" value="' + currentBgHex.toUpperCase() + '">' +
            '<input type="color" id="bg-cp" value="' + currentBgHex + '">' +
          '</div>';
        ...
        alphaRow.innerHTML =
          ...
          '<input type="range" id="bg-alpha" min="0" max="1" step="0.01" value="' + currentAlpha + '">' +
          '<span id="bg-alpha-val" style="font-size:10px;color:#a1a1aa;min-width:32px;text-align:right;">' + Math.round(currentAlpha*100) + '%</span>';
        ...
        if (colors.bgVar) {
          ...
          badge.style.cssText = 'display:flex;align-items:center;gap:5px;margin-bottom:6px;';
          badge.innerHTML =
            '<span style="font-size:9px;color:#71717a;flex:1;">Привязан к</span>' +
            '<span style="background:#1c3553;color:#60a5fa;font-size:9px;padding:2px 7px;border-radius:4px;font-family:monospace;">' + colors.bgVar + '</span>' +
        ...
        function applyRgbaBg() {
          const val = 'rgba(' + parseInt(currentBgHex.slice(1,3),16) + ',' + parseInt(currentBgHex.slice(3,5),16) + ',' + parseInt(currentBgHex.slice(5,7),16) + ',' + currentAlpha + ')';
        ...
        bgCp.addEventListener('input', e => { currentBgHex = e.target.value; bgTx.value = currentBgHex.toUpperCase(); applyRgbaBg(); });
        bgTx.addEventListener('change', e => { if(isValidHex(e.target.value)){currentBgHex=e.target.value;bgCp.value=currentBgHex;applyRgbaBg();} });
        bgAl.addEventListener('input', e => { currentAlpha = parseFloat(e.target.value); bgAlVal.textContent = Math.round(currentAlpha*100)+'%'; applyRgbaBg(); });
      } else {
        addSolidRow(ctrls, 'Фон', colors.bgColor, nv => {
    ...
    if (!colors.textIsGradient) {
      ...
      addSolidRow(ctrls, 'Текст', colors.color, nv => {
  ...
  function addSolidRow(container, label, computedColor, onChange, varName) {
    const rawHex = (computedColor && computedColor !== 'rgba(0, 0, 0, 0)' && computedColor !== 'transparent')
      ? rgbToHex(computedColor) : '#000000';

    const wrapper = document.createElement('div');

    if (varName) {
      const badge = document.createElement('div');
      badge.style.cssText = 'display:flex;align-items:center;gap:5px;margin-bottom:4px;flex-wrap:wrap;';
      badge.innerHTML =
        '<span style="font-size:9px;color:#71717a;flex:1;min-width:50px;">Привязан к</span>' +
        '<span style="background:#1c3553;color:#60a5fa;font-size:9px;padding:2px 7px;border-radius:4px;font-family:monospace;white-space:nowrap;">' + varName + '</span>' +
    ...
    row.innerHTML =
      ...
      '<div class="color-group">' +
        '<input type="text" class="hex" id="' + txId + '" value="' + rawHex.toUpperCase() + '">' +
        '<input type="color" id="' + cpId + '" value="' + rawHex + '">' +
      '</div>';
  ...
  function buildShadowSection(container, el, sel) {
    ...
    const syncColHex = () => { colHex.value = colInput.value.toUpperCase(); };
    colInput.addEventListener('input', () => { syncColHex(); applyShad(); });
    colHex.addEventListener('change', () => { if (isValidHex(colHex.value)) { colInput.value = colHex.value; applyShad(); } });
    [alphaIn, xIn, yIn, blurIn, spreadIn].forEach(i => i.addEventListener('input', applyShad));
  ...
  shadow.getElementById('btn-clear').addEventListener('click', () => {
    if (confirm('Сбросить все изменения и перезагрузить страницу?')) location.reload();
  });
  ...
  shadow.getElementById('btn-stop').addEventListener('click', () => {
    try { document.querySelector('.hero-swiper').swiper.autoplay.stop(); } catch {}
  });
  shadow.getElementById('btn-start').addEventListener('click', () => {
    try { document.querySelector('.hero-swiper').swiper.autoplay.start(); } catch {}
  });
  ...
  shadow.getElementById('imp-file').addEventListener('change', e => {
    ...
    reader.onload = ev => {
      try {
        ...
        if (data.changes) {
          Object.assign(changes.variablesLight, data.changes.variablesLight || data.changes.variables || {});
          Object.assign(changes.variablesDark,  data.changes.variablesDark  || {});
          (data.changes.overrides || []).forEach(o => {
            if (!changes.overrides.find(ex => ex.selector === o.selector && ex.prop === o.prop)) {
